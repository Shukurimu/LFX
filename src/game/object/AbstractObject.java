...
abstract class AbstractObject implements Observable {
  ...
  protected void transitFrame(Action action) {
    ...
    System.out.printf("transitFrame %s (%d)%n", action, transition);
    ...
    for (Tuple<Bdy, Area> bdyArea : that.getBdys()) {
      ...
      for (Tuple<Itr, Area> itrArea : itrList) {
        ...
        if (!bdy.interactsWith(itr, scopeView)) {
          System.out.printf("%s Itr %s -> %s Bdy %s <%d> not interact %n", this, itrArea.second, that, area, scopeView);
          continue;
        }
        System.out.printf("%s Itr %s -> %s Bdy %s <%d> interact %n", this, itrArea.second, that, area, scopeView);
        return itr;
      }
    }
    return null;
  ...
  public String toString() {
    return String.format("%s(%d)@%d", identifier, teamId, this.hashCode());
  }
